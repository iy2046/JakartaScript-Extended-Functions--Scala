...
object hw07 extends JsApp:
  ...
  def compressFold[A](l: List[A]): List[A] = l.foldRight(Nil: List[A]){
    (h, acc) => if (acc.isEmpty || h != acc.head) then h :: acc else acc
  }
  ...
  enum Tree:
    ...
    def foldLeft[A](z: A)(f: (A, Int) => A): A =
      def loop(acc: A, t: Tree): A = t match
        case Empty => acc
        case Node(l, d, r) =>
          val leftCount = loop(acc, l)
          val accTwo = f(leftCount, d)
          loop(accTwo, r)
      loop(z, this)
  ...
  def strictlyOrdered(t: Tree): Boolean =
    val (b, _) = t.foldLeft((true, None: Option[Int])) {
      case ( (acc, None), d ) => ( acc, Some(d) )
      case ( (acc, Some(prev)), d ) => if (prev > d) then ( (acc && true), Some(d) ) else ( false, Some(d) )
    }
    b
      ...
      case Call(e0, es) => 
        e0 match {
          case Function(p, ys, eb) => {
            if ( ys.contains(x) || p.exists(_ == x) ) {
              Call(substX(e0), es.map(substX))
            } else {
              Call(substX(e0), es.map(substX))
            }
          }
          case _ => Call(substX(e0), es.map(substX))
        }
      case Function(p, ys, eb) => 
        if (x == p.getOrElse("") || ys.contains(x)) {
          Function(p, ys, eb)
        } else {
          Function(p, ys, substX(eb))
        }
